...
- [x] 1. CDK プロジェクトの初期化
  ...
  - [x] 1.1 CDK TypeScript プロジェクトを作成する
    - `cdk init app --language typescript` でプロジェクトを初期化
    - 不要なサンプルコードを削除
    - _Requirements: 1.1_
  - [x] 1.2 Biome の設定を追加する
    - `biome.json` を作成
    - フォーマットとリンターのルールを設定
    - package.json にスクリプトを追加
    - _Requirements: 1.2_
  - [x] 1.3 Jest テスト環境を設定する
...
- [x] 3. Storage Construct の実装
  ...
  - [x] 3.3 Minutes_Bucket を作成する
    - S3 Bucket L2 コンストラクトを使用
    - `removalPolicy: DESTROY` と `autoDeleteObjects: true` を設定
    - _Requirements: 2.2, 2.3, 2.6_
  - [x] 3.4 Session_Table を作成する
...
- [x] 5. CDK アプリエントリーポイントの更新

  - [x] 5.1 bin/app.ts を更新する
    - MainStack をインポート
    - 環境設定を追加
    - _Requirements: 1.4_
